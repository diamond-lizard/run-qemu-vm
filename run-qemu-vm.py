...
def find_uefi_bootloader(seven_zip_executable, iso_path):
    ...
    try:
        # Use '7z l' to list files in the EFI/BOOT directory
        result = subprocess.run(
            [seven_zip_executable, 'l', iso_path, 'EFI/BOOT/'],
    ...
    # Search for the standard AArch64 bootloader file in the output.
    for line in result.stdout.splitlines():
        if line.strip().lower().endswith("bootaa64.efi"):
            match = re.search(r'(\S+bootaa64\.efi)$', line.strip(), re.IGNORECASE)
            if match:
                bootloader_name = match.group(1)
                print(f"Info: Found UEFI bootloader: {bootloader_name}")
...
def create_and_run_uefi_with_automation(base_args, config):
    ...
    """
    bootloader = find_uefi_bootloader(config['seven_zip_executable'], config['cdrom'])

    if bootloader:
        with tempfile.TemporaryDirectory(prefix="qemu-uefi-boot-") as temp_dir:
            startup_script_path = Path(temp_dir) / "startup.nsh"
            # UEFI shell uses backslashes for paths
            bootloader_path = f"EFI\\BOOT\\{bootloader}"
